...
type
  TFormMotsCoranTZ = class(TForm)
    ...
    procedure Drid1TitleClick(Column: TColumn);
...
// Le nombre obtenu sera sur le fichier texte 'FileHarfTZ.txt'
//
...
Begin
  ...
  AssignFile(F2,'FileHarfTZ.txt');
  Rewrite(F2);
  //i := 1;
  //
  DMZadM.FDQTabHarfTZ.first;
  // Copie de la base  'FDQTabHarfTZ'  sur le le fichier texte  'FileHarfTZ.txt'
...
// تعبئة حروف القرآن
//
// Remplissage des champs de poistion des HARFs
//
...
Begin
  ...
  With ProgressBar1 do
    begin
      ...
      nMot  := DMZadM.FDTabHarfTZ.FieldByName('MOTNO').AsInteger;               // رقم الآية
      //
      For i := Min to Max do
       Begin
        Position := i;
        ...
        DMZadM.FDTabHarfTZ.FieldByName('PHQ').AsInteger := iPHQ;
  ...
  case RadioGroup1.ItemIndex of
    ...
    begin
      ...
      if SaveDialog1.Execute then
        SaveToExcelFile(sPath + SaveDialog1.FileName);
    end;
    1:
    begin
      SaveDialog1.Filter := 'Comma Delimited|*.csv';
      if SaveDialog1.Execute then
        SaveToCSVFile(sPath + SaveDialog1.FileName);
    end;
  end;
end;
...
//
// Somme des codes de caratères du Coran
//
...
//
procedure TFormMotsCoranTZ.Drid1TitleClick(Column: TColumn);
Var
  StrSql : String;
Begin
  //
